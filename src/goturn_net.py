...
class TRACKNET:
    def __init__(self, batch_size, train=True):
        ...
        self.parameters = {}
        self.outdim_cord = POLICY['side'] * POLICY['side'] * POLICY['num'] * 4
    ...
    def build(self):
        ...
        self.re_fc4_adj = tf.reshape(self.fc4_adj, shape=[-1, POLICY['side'], POLICY['side'], 4])

        self.net_out = tf.concat([self.re_fc4_adj, self.re_fc4_image], axis=3, name="net_out")
        tf.summary.image("objectness", expit_tensor(self.net_out[:, :, :, 4:]), max_outputs=4)

        self.print_shapes()

        if (self.train):
            self.loss = self.loss_grid(self.net_out, POLICY, name="loss")
    ...
    def _loss_layer(self, bottom, label, name=None):
        diff = tf.subtract(self.fc4_concat, self.bbox)
